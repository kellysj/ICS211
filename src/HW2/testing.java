 ...
 * @author Jones,Kelly
 */
public class testing<E extends Comparable<E>> {
	public static void main(String args[]){
		System.out.println("INTEGER TESTS");
		testing<Integer> testInteger = new testing<Integer>();
	...
	public void go(int n, String testprefix){
		...
		try{
			...
			for(int i = 0; i<n;i++){
				testObject.add((E)(Integer.valueOf(((int)(Math.random()*10)))));
			}
		...
		try{
			...
			System.out.println(testprefix + ":\t" +  "checking get(" + n/2 + ") value is " + testObject.get(n/2));//seeing if method works inside the size of the list
			failOut = "failed on get(0)";
			System.out.println(testprefix + ":\t" +  "checking get(" + 0 + ") value is " + testObject.get(0));//seeing if it works at 0
			failOut = "failed on get(n*2)";
		...
		try{
			...
			if(compD.compare((E)testObject.get(n/2),to_set)==0){//compares the index of the set to the set value, checking if the value was set
				System.out.println(testprefix + ":\t" +  "no index set test: " + testObject.get(n/2) + "=" + to_set + " set() successful! List is: " + testObject.printlist());
			}
			else{
				System.out.println(testprefix + ":\t" +  "no index set test: " + testObject.get(n/2) + "!=" + to_set + " set() failed!");
			}
		} catch(IndexOutOfBoundsException e){//should be thrown in 0 length cases
			System.out.println(testprefix + ":\t" +  "unindexed add: expected exception to throw for 0 length, otherwise something is broken");
		}
